...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (typeof url !== 'string' || !url.startsWith(process.env.NEXT_PUBLIC_REMOTE_IMAGE_URL ?? '')) {
    logger.error(`[REQ-API] /api/image/blur, 잘못된 이미지 url 요청 with '${url}'`);
  ...
  try {
    ...
    logger.info(`[REQ-API] /api/image/blur, '${url}' blur image created`);
    return res.status(200).json({
      blurDataURL,
      img,
    });
  } catch (e) {
    logger.error(`[REQ-API] /api/image/blur, blur 이미지 생성 실패 with '${url}', cause: ${JSON.stringify(e)}`);
