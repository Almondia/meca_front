...
import type { MyProfile, UserUpdateRequest } from '@/types/domain/user';
...
const useProfileUpdate = () => {
  ...
  const { mutate: mutateUserProfile } = useMutation<never, unknown, UserUpdateRequest, { previousUser?: MyProfile }>(
    ['updateuser'],
    async ({ name, profile }) => userApi.updateProfile({ name, profile }),
    {
